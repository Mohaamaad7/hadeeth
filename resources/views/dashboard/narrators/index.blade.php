                    ...
                    <div class="col-md-5">
                        <div class="form-group">
                            <input type="text" name="search" class="form-control" 
                                   placeholder="ابحث في الاسم، السيرة، أو الدرجة..." 
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <select name="grade_status" class="form-control">
                                ...
                                <option value="صحابي" {{ request('grade_status') === 'صحابي' ? 'selected' : '' }}>صحابي</option>
                                ...
                                <option value="متروك" {{ request('grade_status') === 'متروك' ? 'selected' : '' }}>متروك</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-3">
                    ...
                    <tbody>
                        @foreach($narrators as $narrator)
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        <a href="{{ route('dashboard.narrators.show', $narrator) }}" 
                                           class="btn btn-sm btn-info" title="عرض">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('dashboard.narrators.edit', $narrator) }}" 
                                           class="btn btn-sm btn-warning" title="تعديل">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button type="button" 
                                                class="btn btn-sm btn-danger" 
                                                onclick="confirmDelete({{ $narrator->id }})"
                                                title="حذف">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </div>
                                    <form id="delete-form-{{ $narrator->id }}" 
                                          action="{{ route('dashboard.narrators.destroy', $narrator) }}" 
                                          method="POST" 
                                          style="display: none;">
                                        @csrf
                                        @method('DELETE')
