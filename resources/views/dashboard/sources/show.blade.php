...
<div class="row">
    <div class="col-md-4">
        ...
        <form id="delete-form" action="{{ route('dashboard.sources.destroy', $source) }}" method="POST"
            style="display: none;">
            @csrf
            @method('DELETE')
        </form>
        ...
        <div class="card card-success">
            <div class="card-header">
                ...
                <div class="card-tools">
                    <a href="{{ route('dashboard.hadiths.create') }}" class="btn btn-sm btn-success">
                        ...
                        <tbody>
                            @foreach($source->hadiths as $hadith)
                                            <tr>
                                                ...
                                                <td>
                                                    @if($hadith->narrators->count() > 0)
                                                        <small
                                                            class="text-muted">{{ $hadith->narrators->pluck('name')->map(fn($n) => Str::limit($n, 20))->join('، ') }}</small>
                                                    @else
                                ...
                                ($hadith->grade === 'حسن' ? 'info' : 'warning') 
                                                ...
                                                <td>
                                                    <a href="{{ route('dashboard.hadiths.show', $hadith) }}" class="btn btn-xs btn-info">
