                    ...
                    <div class="col-md-5">
                        <div class="form-group">
                            <input type="text" name="search" class="form-control" 
                                   placeholder="ابحث في الاسم، الرمز، أو النوع..." 
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <select name="type" class="form-control">
                                ...
                                <option value="كتب الصحيح" {{ request('type') === 'كتب الصحيح' ? 'selected' : '' }}>كتب الصحيح</option>
                                <option value="كتب السنن" {{ request('type') === 'كتب السنن' ? 'selected' : '' }}>كتب السنن</option>
                                <option value="كتب المسانيد" {{ request('type') === 'كتب المسانيد' ? 'selected' : '' }}>كتب المسانيد</option>
                                <option value="كتب المعاجم" {{ request('type') === 'كتب المعاجم' ? 'selected' : '' }}>كتب المعاجم</option>
                                <option value="كتب التاريخ" {{ request('type') === 'كتب التاريخ' ? 'selected' : '' }}>كتب التاريخ</option>
                                <option value="كتب الأجزاء" {{ request('type') === 'كتب الأجزاء' ? 'selected' : '' }}>كتب الأجزاء</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-3">
                    ...
                    <tbody>
                        @foreach($sources as $source)
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        <a href="{{ route('dashboard.sources.show', $source) }}" 
                                           class="btn btn-sm btn-info" title="عرض">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('dashboard.sources.edit', $source) }}" 
                                    ...
                                    <form id="delete-form-{{ $source->id }}" 
                                          action="{{ route('dashboard.sources.destroy', $source) }}" 
                                          method="POST" 
                                          style="display: none;">
                                        @csrf
                                        @method('DELETE')
