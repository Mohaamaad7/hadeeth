...
<article
    class="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 p-6 border-r-4 border-[--color-accent]">
    <!-- Hadith Number & Grade -->
    <div class="flex items-center justify-between mb-4">
        <div class="flex gap-2">
            @if($hadith->number_in_book)
                <span
                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-[--color-charcoal] text-[--color-paper]">
        ...
        <div class="flex gap-1">
            @foreach($hadith->sources as $source)
                <span
                    class="w-8 h-8 rounded-full bg-[--color-accent] text-white flex items-center justify-center text-sm font-bold"
                    title="{{ $source->name }}">
                    {{ $source->code }}
                </span>
            @endforeach
    ...
    <div class="flex items-center justify-between text-sm text-gray-600 border-t pt-4">
            ...
            @if($hadith->narrators->count() > 0)
                @foreach($hadith->narrators as $nar)
                    <span class="flex items-center gap-2">
                        ...
                        <span>الراوي: <strong>{{ $nar->name }}</strong></span>
                    </span>
                @endforeach
            @endif
        ...
        <div class="flex gap-2">
            <button @click="copyToClipboard('{{ addslashes($hadith->content) }}')"
                class="p-2 rounded-full hover:bg-gray-100 transition-colors" title="نسخ النص">
            ...
            <button @click="shareHadith({{ $hadith->id }})" class="p-2 rounded-full hover:bg-gray-100 transition-colors"
                title="مشاركة">
                <svg class="w-5 h-5 text-[--color-accent]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    @if($hadith->explanation)
        <div class="mt-4 pt-4 border-t border-gray-200" x-data="{ expanded: false }">
            <button @click="expanded = !expanded" class="flex items-center gap-2 text-[--color-accent] font-medium text-sm">
                ...
                <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': expanded }" fill="currentColor"
                    viewBox="0 0 20 20">
