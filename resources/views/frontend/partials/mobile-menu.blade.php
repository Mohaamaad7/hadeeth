{{-- ╔═══════════════════════════════════════════════════════════════╗ --}}
{{-- ║ Mobile Menu — موسوعة الحديث الصحيح ║ --}}
{{-- ║ Unified slide-over menu for mobile devices ║ --}}
{{-- ╚═══════════════════════════════════════════════════════════════╝ --}}
...
<div id="mobile-menu"
    ...
    <nav class="flex-1 overflow-y-auto py-3 px-3">
        ...
        @foreach($mobileNavItems as $index => $item)
            ...
            @endphp
            <a href="{{ route($item['route']) }}" class="flex items-center gap-4 p-3.5 rounded-2xl mb-1 transition-all duration-300 group
                          {{ $isActive
            ? 'bg-emerald-50 text-emerald-700'
            : 'text-gray-600 hover:bg-gray-50' }}"
                style="animation: slideInRight 0.4s ease-out {{ ($index * 0.06) }}s both;">
                <div
            ...
            : 'bg-gray-100 text-gray-500 group-hover:bg-emerald-100 group-hover:text-emerald-600' }}">
                ...
                <div class="flex-grow">
                    <div class="font-bold text-sm {{ $isActive ? 'text-emerald-800' : '' }}">{{ $item['label'] }}</div>
                    <div class="text-[11px] {{ $isActive ? 'text-emerald-600/70' : 'text-gray-400' }}">{{ $item['desc'] }}
                    </div>
                </div>
                @if($isActive)
