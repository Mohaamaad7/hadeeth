{{-- ╔═══════════════════════════════════════════════════════════════╗ --}}
{{-- ║ Header / Navbar — موسوعة الحديث الصحيح ║ --}}
{{-- ║ Unified across all frontend pages ║ --}}
{{-- ╚═══════════════════════════════════════════════════════════════╝ --}}
...
<nav id="main-navbar"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between {{ $isHome ? 'h-20' : 'h-16' }} items-center transition-all duration-300">
            {{-- Logo --}}
            ...
            <div class="hidden lg:flex items-center gap-1">
                ...
                @foreach($navItems as $item)
                            ...
                            @endphp
                            <a href="{{ route($item['route']) }}" class="relative px-4 py-2 rounded-xl text-sm font-bold transition-all duration-300 group
                                          {{ $isActive
                    ? 'text-emerald-700 bg-emerald-50'
                    : 'text-gray-500 hover:text-emerald-600 hover:bg-emerald-50/50' }}">
                                <i
                                    class="fa-solid {{ $item['icon'] }} ml-1.5 text-xs {{ $isActive ? 'text-emerald-500' : 'text-gray-400 group-hover:text-emerald-400' }} transition-colors"></i>
                                {{ $item['label'] }}
                                @if($isActive)
                                    <span
            ...
            <div class="flex items-center gap-2">
                {{-- Search Bar (compact - for non-home pages) --}}
                @unless($isHome)
