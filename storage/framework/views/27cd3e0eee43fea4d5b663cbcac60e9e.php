...
<?php $__env->startSection('meta_keywords', 'حديث رقم ' . $hadith->number_in_book . ', ' . ($hadith->narrator?->name ?? '') . ', ' . ($hadith->book?->name ?? '') . ', حديث ' . $hadith->grade . ', الأحاديث النبوية'); ?>
...
<script type="application/ld+json">
{
    ...
    "author": {
        "@type": "Person",
        "name": "<?php echo e($hadith->narrator?->name ?? 'غير محدد'); ?>"
    },
    "publisher": {
        "@type": "Organization",
    ...
    "keywords": ["حديث", "<?php echo e($hadith->grade); ?>", "<?php echo e($hadith->narrator?->name ?? ''); ?>", "<?php echo e($hadith->book?->name ?? ''); ?>"]
}
</script>
<script type="application/ld+json">
{
...
<div class="container mx-auto px-4 py-8 max-w-5xl">
    ...
    <section class="bg-white rounded-3xl shadow-xl overflow-hidden mb-8 animate-up border border-gray-100 relative">
        ...
        <div class="p-8 md:p-12 relative">
            ...
            <div class="flex flex-wrap gap-3 mb-6 relative z-10">
                <span
                    class="bg-emerald-50 text-emerald-800 px-4 py-1.5 rounded-full text-sm font-bold border border-emerald-200">
                    <i class="fa-solid fa-hashtag ml-1 text-emerald-500"></i> حديث رقم:
                ...
                ?>
                <span
                    class="bg-<?php echo e($gradeColor); ?>-50 text-<?php echo e($gradeColor); ?>-700 px-4 py-1.5 rounded-full text-sm font-bold border border-<?php echo e($gradeColor); ?>-200">
                    <i class="fa-solid fa-check-circle ml-1"></i> <?php echo e($hadith->grade); ?>

                </span>
                <?php if ($hadith->narrator): ?>
                    <span
                        class="bg-gray-50 text-gray-600 px-4 py-1.5 rounded-full text-sm font-bold border border-gray-200">
                        <i class="fa-solid fa-user ml-1 text-gray-400"></i> الصحابي:
                ...
                <?php if ($hadith->book): ?>
                    <span
                        class="bg-purple-50 text-purple-600 px-4 py-1.5 rounded-full text-sm font-bold border border-purple-200">
            ...
            <div class="hadith-frame bg-emerald-50/30 p-8 md:p-10 rounded-2xl text-center my-6">
                ...
                <p
                    class="font-scheherazade text-2xl md:text-3xl leading-[3.5] text-gray-800 text-justify md:text-center relative z-10">
            ...
            <?php if (!empty($hadith->additions) && is_array($hadith->additions)): ?>
                <div class="mt-4 mb-6 space-y-3 px-2 md:px-6">
                    <?php $__currentLoopData = $hadith->additions;
                    $__env->addLoop($__currentLoopData);
                    foreach ($__currentLoopData as $addition):
                        $__env->incrementLoopIndices();
                        $loop = $__env->getLastLoop(); ?>
                        <div
                            class="bg-gray-50/80 rounded-xl p-4 md:p-5 border border-gray-100 relative overflow-hidden group hover:border-emerald-200 transition-colors">
                            <div
                                class="absolute right-0 top-0 bottom-0 w-1.5 bg-emerald-400/70 group-hover:bg-emerald-500 transition-colors">
                            </div>
                            <div class="flex flex-col gap-3 pr-3">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-md text-xs font-bold inline-flex items-center gap-1.5">
            ...
            <?php if ($hadith->raw_text): ?>
                <div class="mt-8 mb-4">
                    <button onclick="toggleRawText()" id="rawTextToggle"
                        ...
                        <div class="flex items-center gap-3">
                            <div
                                class="w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center text-amber-700 group-hover:bg-amber-200 transition-colors">
                                <i class="fa-solid fa-scroll text-sm"></i>
                            </div>
                            <div>
                                <span class="font-bold text-gray-800 text-sm">النص كما ورد في المصدر</span>
                                <span class="block text-[10px] text-amber-600 font-medium">الأمانة العلمية — نص الكتاب
                                    الأصلي</span>
                            </div>
                        </div>
                        <i id="rawTextArrow"
                            class="fa-solid fa-chevron-down text-amber-400 transition-transform duration-300 text-sm"
                            style="transform: rotate(180deg);"></i>
                    </button>

                    <div id="rawTextContent" class="overflow-hidden transition-all duration-500 ease-in-out"
                        style="opacity: 1;">
                        <div class="mt-1 rounded-b-xl border border-t-0 border-amber-200/60 overflow-hidden shadow-inner">
                            ...
                            <div class="p-4 md:p-6 relative"
                                style="background: linear-gradient(135deg, #fffdf5 0%, #fef9e7 50%, #fffdf5 100%);">

                                <div
                                    class="absolute top-2 right-2 w-4 h-4 border-t border-r border-amber-400/50 rounded-tr-sm">
                                </div>
                                <div
                                    class="absolute top-2 left-2 w-4 h-4 border-t border-l border-amber-400/50 rounded-tl-sm">
                                </div>
                                <div
                                    class="absolute bottom-2 right-2 w-4 h-4 border-b border-r border-amber-400/50 rounded-br-sm">
                                </div>
                                <div
                                    class="absolute bottom-2 left-2 w-4 h-4 border-b border-l border-amber-400/50 rounded-bl-sm">
                                </div>


                                <p class="font-scheherazade text-lg md:text-xl leading-[2.5] text-amber-950 text-center relative z-10 px-2 md:px-6"
                                    dir="rtl">
                                ...
                                <div class="mt-4 pt-3 border-t border-dashed border-amber-300/50 text-center">
                                    <p
                                        class="text-[10px] text-amber-600/80 font-medium flex items-center justify-center gap-1.5">
                                        <i class="fa-solid fa-book-open text-amber-400"></i>
                                        هذا النص الأصلي كما ورد في كتاب "صحيح الجامع الصغير وزيادته" للإمام الألباني رحمه
                                        الله
                                    </p>
                                </div>
                            </div>
            ...
            <?php if ($hadith->sources->count() > 0): ?>
                <div
                    class="mt-8 pt-6 border-t border-dashed border-gray-200 text-sm text-gray-600 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                    <div class="flex items-center gap-2 flex-wrap">
                        <i class="fa-solid fa-book-bookmark text-emerald-500 text-lg"></i>
                        <span class="font-bold">التخريج:</span>
                        <?php $__currentLoopData = $hadith->sources;
                        $__env->addLoop($__currentLoopData);
                        foreach ($__currentLoopData as $source):
                            $__env->incrementLoopIndices();
                            $loop = $__env->getLastLoop(); ?>
                            <span class="bg-gray-100 px-3 py-1 rounded-lg text-xs font-bold"><?php echo e($source->name); ?>
                                (<?php echo e($source->code); ?>)</span>
                        <?php endforeach;
                        $__env->popLoop();
                        $loop = $__env->getLastLoop(); ?>
                    </div>
                    <div class="flex gap-3">
                        <button onclick="copyHadith()" class="text-gray-400 hover:text-emerald-600 transition-colors"
                            title="نسخ الحديث">
                            <i class="fa-regular fa-copy text-xl"></i>
                        </button>
                        <button onclick="openShareModal()" class="text-gray-400 hover:text-emerald-600 transition-colors"
                            title="مشاركة">
    ...
    <?php if ($hadith->chains->count() > 0): ?>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 animate-up" style="animation-delay: 0.1s;">
            <?php $__currentLoopData = $hadith->chains;
            $__env->addLoop($__currentLoopData);
            foreach ($__currentLoopData as $index => $chain):
                $__env->incrementLoopIndices();
                $loop = $__env->getLastLoop(); ?>
                <section
                    class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:border-emerald-200 transition-colors">
                    <div class="flex items-center gap-3 mb-4 pb-3 border-b border-gray-100">
                        <span
                            class="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center text-emerald-600 font-black text-sm"><?php echo e($index + 1); ?></span>
                        <h3 class="font-tajawal font-bold text-lg text-gray-800">طريق <?php echo e($chain->source->name); ?>
                        </h3>
                    </div>
                    <div class="pr-2">
                        <?php $__currentLoopData = $chain->narrators;
                        $__env->addLoop($__currentLoopData);
                        foreach ($__currentLoopData as $narrator):
                            $__env->incrementLoopIndices();
                            $loop = $__env->getLastLoop(); ?>
                            <div class="timeline-node <?php echo e($loop->last ? '' : 'pb-4'); ?>">
                                ...
                                <?php if ($loop->last): ?>
                                    <span
                                        class="font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-lg"><?php echo e($narrator->name); ?></span>
                                <?php else: ?>
    ...
    <?php if ($hadith->narrator && $hadith->narrator->bio): ?>
        <section class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 animate-up"
            ...
            <div class="flex items-start gap-4">
                <div
                    class="flex-shrink-0 w-16 h-16 rounded-2xl bg-emerald-100 flex items-center justify-center text-emerald-600 text-2xl">
                    <i class="fa-solid fa-user"></i>
                </div>
                <div class="flex-grow">
                    <h4 class="font-bold text-lg text-gray-800 mb-2"><?php echo e($hadith->narrator->name); ?></h4>
                    <?php if ($hadith->narrator->grade_status): ?>
                        <span class="inline-block px-3 py-1 rounded-full text-xs font-bold mb-3"
                            style="background-color: <?php echo e($hadith->narrator->color_code); ?>20; color: <?php echo e($hadith->narrator->color_code); ?>; border: 1px solid <?php echo e($hadith->narrator->color_code); ?>;">
                            <?php echo e($hadith->narrator->grade_status); ?>

                        </span>
                    <?php endif; ?>
                    <p class="text-gray-600 leading-relaxed whitespace-pre-wrap"><?php echo e($hadith->narrator->bio); ?>
                    </p>
                </div>
            </div>
        </section>
    ...
    <?php if ($relatedHadiths->count() > 0): ?>
        <section class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 animate-up"
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <?php $__currentLoopData = $relatedHadiths;
                $__env->addLoop($__currentLoopData);
                foreach ($__currentLoopData as $related):
                    $__env->incrementLoopIndices();
                    $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route('hadith.show', $related->id)); ?>"
                        ...
                        <div class="flex items-center gap-2 mb-3">
                            <span
                                class="text-xs bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full font-bold">#<?php echo e($related->number_in_book); ?></span>
                            <span
                                class="text-xs bg-green-50 text-green-700 px-3 py-1 rounded-full font-bold"><?php echo e($related->grade); ?></span>
                        </div>
...
<div id="shareModal" class="fixed inset-0 z-50 hidden items-center justify-center">
    ...
    <!-- Modal Content -->
    <div class="relative bg-white rounded-3xl shadow-2xl p-6 md:p-8 w-full max-w-md mx-4 transform scale-95 opacity-0 transition-all duration-300"
        ...
        <div class="grid grid-cols-2 gap-3 mb-6">
            ...
            <button onclick="shareToWhatsApp()"
                class="flex items-center justify-center gap-3 bg-[#25D366] hover:bg-[#20bd5a] text-white py-4 px-4 rounded-2xl font-bold transition-all hover:scale-105 shadow-lg shadow-green-200">
            ...
            <button onclick="shareToTwitter()"
                class="flex items-center justify-center gap-3 bg-black hover:bg-gray-800 text-white py-4 px-4 rounded-2xl font-bold transition-all hover:scale-105 shadow-lg">
            ...
            <button onclick="shareToTelegram()"
                class="flex items-center justify-center gap-3 bg-[#0088cc] hover:bg-[#0077b5] text-white py-4 px-4 rounded-2xl font-bold transition-all hover:scale-105 shadow-lg shadow-blue-200">
            ...
            <button onclick="shareToFacebook()"
                class="flex items-center justify-center gap-3 bg-[#1877F2] hover:bg-[#166fe5] text-white py-4 px-4 rounded-2xl font-bold transition-all hover:scale-105 shadow-lg shadow-blue-200">
        ...
        <button onclick="copyLinkOnly()"
            class="w-full flex items-center justify-center gap-3 bg-gray-100 hover:bg-gray-200 text-gray-700 py-4 px-4 rounded-2xl font-bold transition-all">
...
<script>
    ...
    function copyHadith() {
        ...
        const narrator = `<?php echo e($hadith->narrator?->name ?? 'غير محدد'); ?>`;
